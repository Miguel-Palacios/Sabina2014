...
//     g_Context.InitFromXmlFile("../data/SamplesConfig.xml", g_scriptNode, &errors);
    ...
    ((XnVSessionManager*)pSessionGenerator)->Initialize(&g_Context, "RaiseHand", "RaiseHand");
...
void Ni_Device::changePrimaryStream(int nIndex)
{
    ...
    case XN_NODE_TYPE_GESTURE:
        g_pPrimary = *getGestureGenerator();
        break;
    case XN_NODE_TYPE_HANDS:
        g_pPrimary = *getHandsGenerator();
        break;
    }
}
...
bool Ni_Device::isUserOn()
{
  return (Ni_Device::g_bIsUserOn);
}

bool Ni_Device::isGestureOn()
{
  return (Ni_Device::g_bIsGestureOn);
}

bool Ni_Device::isHandsOn()
{
  return (Ni_Device::g_bIsHandsOn);
}


bool Ni_Device::isSceneOn()
{
  return (Ni_Device::g_bIsSceneOn);
}
...
GestureGenerator* Ni_Device::getGestureGenerator()
{
    return g_Gesture.IsValid() ? &g_Gesture : NULL;
}

HandsGenerator* Ni_Device::getHandsGenerator()
{
    return g_Hands.IsValid() ? &g_Hands : NULL;
}
...
SceneMetaData* Ni_Device::getSceneMetaData()
{
 return g_Scene.IsValid() ? &g_SceneMD : NULL;
}
